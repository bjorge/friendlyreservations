...
mutation ExportCSV(
    $propertyId: String!) {
        exportCSV(propertyId: $propertyId) {
        eventVersion
        }
  }
`
    ...
    render() {
        ...
        return (
            <Modal isOpen={showModal} toggle={this.toggle}>
                ...
                <ModalBody>
                    To export CSV files click the export button and then check your email for CSV data.
                    <hr />
                    <Mutation client={apolloClient} mutation={EXPORT_GQL_MUTATION} fetchPolicy='no-cache'

                        onCompleted={(data) => {
                            if (data.export !== undefined) {
                                this.props.appStateStore.setPropertyEventVersion(data.exportCSV.eventVersion);
                            }
                            this.toggle();
                        }}>
...
export default inject('appStateStore')(observer(ExportCSV))
